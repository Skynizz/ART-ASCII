...
output_format = input("Voulez-vous enregistrer en (JPG/TXT) ? ").strip().lower()

ascii_art = map_pixels_to_ascii(grayscale_image(resize_image(Image.open(image_path))))

if output_format == "jpg":
    output_path = input("Entrez le nom du fichier de sortie (ex: output.jpg) : ")
    save_ascii_art_to_image(ascii_art, output_path)
    print(f"Art ASCII sauvegardé en {output_path}")
elif output_format == "txt":
    output_path = input("Entrez le nom du fichier de sortie (ex: output.txt) : ")
    save_ascii_art_to_text(ascii_art, output_path)
    print(f"Art ASCII sauvegardé en {output_path}")
else:
    print("Format non reconnu. Veuillez choisir JPG ou TXT.")
